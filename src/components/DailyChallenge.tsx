  ...
  return (
    <div id="daily-challenge">
      <div className="border-b border-[var(--color-line-soft)] pb-[22px] mb-8">
        ...
        <p className="text-[var(--color-fg-muted)] text-[11px] mt-2">Una prueba fija de 20 palabras, una vez al día</p>
      </div>

      {!isCompleted ? (
        <div className="flex justify-between items-center gap-10 flex-wrap" id="daily-challenge-pending">
          <div className="flex-1 min-w-[280px]">
            <div className="text-[9px] tracking-[0.2em] text-[var(--color-fg-dim)] uppercase mb-4">Hoy · 20 palabras</div>
            <div className="display-heavy text-[30px] leading-[1.15]">Poné a prueba tu intuición diaria</div>
            <p className="text-[var(--color-fg-muted)] text-xs mt-3.5 leading-relaxed max-w-[420px]">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} id="daily-challenge-completed">
          <div className="flex justify-between items-baseline mb-2">
            <span className="text-[11px] text-[var(--color-fg-soft)]">Completado hoy — vuelve mañana</span>
          </div>
          ...
          <div className="grid grid-cols-3 border-t border-b border-[var(--color-line-soft)] mt-5">
            <div className="py-[26px] text-center border-r border-[var(--color-line-soft)]">
              <div className="text-[9px] tracking-[0.2em] text-[var(--color-fg-dim)] uppercase">Puntuación</div>
            ...
            <div className="py-[26px] text-center">
              <div className="text-[9px] tracking-[0.2em] text-[var(--color-fg-dim)] uppercase">Recompensa</div>
          ...
          <div className="flex gap-1.5 items-end h-[70px] mt-8">
            {history.map((h) => (
              <div key={h.dateStr} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                ...
                <span className="text-[9px] text-[var(--color-fg-dim)]">{h.label}</span>
              </div>
            ))}
          </div>
        </motion.div>
